...
try:
    ...
    for dt_set in dt_sets:
        ...
        for kwd in kwd_with_rank:
            try:
                ...
                cut_by_year_ndla_s = DateUtil.cut_by_year(api_r_ndla, PERIODS_4_BRAND)

                ## analyze std & growth rate
                Dla = DataLabAnalysis(URI['NAVER_DATALAB']['CAT_BY_KW'], cut_by_year_ndla_s)
                g_rate = Dla.get_growth_rate()

                ll_r['cat_code'] = dt_set['cat_code']
                ll_r['keyword'] = keyword
                ll_r['rank'] = rank
                ll_r['growth_rate'] = g_rate
    ...
    FsUtil.save_json_2_csv_file(result, root_path + "/public/output/test.csv")
except Exception as e:
    raise e
